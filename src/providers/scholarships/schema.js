...
const SCHOLARSHIP_RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    program_format: {
      type: SchemaType.STRING,
      description: 'Must evaluate to exactly one of these: "Research/Thesis-based", "Coursework-based", or "Unknown".'
    },
    deadline: {
      type: SchemaType.STRING,
    ...
    pakistan_eligible: {
      ...
      description: 'True if Pakistani citizens are explicitly eligible or included in the international applicant pool.'
    },
    application_fee: {
      type: SchemaType.STRING,
    ...
    spouse_allowance: {
      type: SchemaType.BOOLEAN,
      description: 'True if the grant explicitly provides allowances, stipends, or visa support for spouses/dependents.'
    },
    english_taught: {
      type: SchemaType.BOOLEAN,
  ...
  required: [
    'match_score', 'uni_country', 'scholarship_name', 'program_name', 'program_format',
    'deadline', 'pakistan_eligible', 'application_fee', 'test_requirements', 'cgpa_requirement',
    'work_experience', 'fully_funded', 'block_account', 'spouse_allowance', 'english_taught',
    'post_study_visa', 'verdict', 'apply_link', 'official_link'
  ]
};
