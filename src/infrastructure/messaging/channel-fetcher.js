...
class ChannelFetcher extends EventEmitter {
  ...
  async fetchWithRetry() {
    const strategies = [
      {
        label: 'Newsletter Store (window.Store.Newsletter)',
        fn: this._strategyNewsletterStore.bind(this),
      },
    ...
    try {
      await pupPage.evaluate((bridgeFnName) => {
        ...
        function extractData(n) {
          ...
          return {
            id,
            name: n.name || n.metadata?.name || n.formattedName || n.displayName || '',
  ...
  async _strategyNewsletterStore() {
    return this.client.pupPage.evaluate(() => {
      ...
      for (const store of stores) {
        const iterate =
          typeof store.forEach === 'function'
            ? (fn) => store.forEach(fn)
            : Array.isArray(store.models)
              ? (fn) => store.models.forEach(fn)
              : typeof store.getAll === 'function'
                ? (fn) => store.getAll().forEach(fn)
                : null;
  ...
  async _strategyWAPI() {
    return this.client.pupPage.evaluate(() => {
      if (!window.WAPI?.getNewsletterSubscriptions) {
        return [];
      }
      try {
  ...
  _handleRawChannelData(rawData) {
    ...
    if (!rawData.id.endsWith(NEWSLETTER_SUFFIX)) {
      return;
    }
